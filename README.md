...
很多业务用**固定 / 零 IV**, 真实 IV 不在密文里 —— 这时整段都是密文, 不能把开头 16
字节当 IV 切掉。KeyHound 对 CBC 系算法会**同时尝试两种约定**:

- **前置 IV**: 密文 = `IV(16) ‖ 密文体`, 解出完整明文;
- **无前置 IV / 零 IV**: 整段都是密文, 用零 IV 解。CBC 里 IV 只异或到第一个明文块,
  第 2 块往后只依赖前一个密文块, 所以**只要 KEY 对, 除首块 16 字节外的明文全部正确**,
  PKCS#7 padding 也照常校验。

**首块 (前 16/8 字节) 自动反查 IV**: 找到 KEY 后, 用 `D = Decrypt(K, C₀) = P₀ XOR IV`,
在 dump 里扫一遍找出能让 `D XOR W` 变成合法 JSON 头 (`{"…`) 的 16 字节窗口 W —— 写死的
固定 IV 几乎总在内存里, 命中即真实 IV, 首块明文随之补全 (命中卡片显示完整 IV 和原文)。
反查不到时, 首块标记为「首块未知」, 其余明文照常给出。

> 所以**不需要也无法**手填一个拿不到的 IV：KEY 一对, 绝大部分明文直接出来, 首块再从
> dump 里把 IV 捞回来补全。含中日韩字符的 JSON 明文也不会再被 ASCII 阈值误杀。
